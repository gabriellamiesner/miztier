...
const browser = await puppeteer.launch();
    ...
    var movies = await page.evaluate(() => {
			...
			let n = [];
			for (i in stri){ if (stri[i].startsWith('IEPW')){ n.push(stri[i]) }}
      ...
      fs.appendFile("./data/courses.json", JSON.stringify(movies, null, 3),  (err) => {
        if (err) {
            console.error(err);
            return;
        };
...
scrape("http://catalog.missouri.edu/courseofferings/iepw")
